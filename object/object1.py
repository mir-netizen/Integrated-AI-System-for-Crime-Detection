...
# Load weapon detection model (custom trained for guns and knives)
import os
model_path = os.path.join(os.path.dirname(__file__), 'knifegun1.pt')
model = YOLO(model_path)
print("Weapon Detection Service is running...")
...
for message in consumer:
    ...
    if detections:
        ...
        print(f"Detected {len(detections)} weapons for clip {data.get('clip_id', 'N/A')[:12]} frame {data.get('frame_index', 'N/A')}")
